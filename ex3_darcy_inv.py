...
def main():
    ...
    model_name, result_name = get_model_name(model='darcy_inv',
...
# #     np.save("time darcy_inv original galerkin per epoch", a)
#     np.save("time darcy_inv svd %s k=%d per epoch"%(args.attention_type,k), a)
#     print(a)
#     time = result["time"]
#     time = np.mean(time)
# #     np.save("darcy_inv mean time svd %s k=%d "%(args.attention_type, k), time)
#     np.save("darcy_inv mean time original %s "%args.attention_type, time)
 ...
 #   np.save("best metric darcy_inv svd %s k=%d"%(args.attention_type, k), val_metric)
    
    
#     loss_train = result['loss_train']
#     loss_val = result['loss_val']
#     np.save("darcy_inv_original_%s_train_loss"%args.attention_type, loss_train)
#     np.save('darcy_inv_original_%s_val_loss'%args.attention_type, loss_val)

#     np.save("darcy_inv_%d_svd_%s_train_loss"%(k, args.attention_type), loss_train)
#     np.save('darcy_inv_%d_svd_%s_val_loss'%(k, args.attention_type), loss_val)
...
#     np.save("inference time darcy_inv svd %s k=%d"%(args.attention_type,k), mean_syn) 
    ...
    with torch.no_grad():
        ...
        preds = _out['preds']
        attn = _out['attn_weights']
    torch.save(attn, "darcy_inv attention_1") 
