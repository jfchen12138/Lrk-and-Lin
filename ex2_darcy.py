...
def main():
    ...
    model_name, result_name = get_model_name(model='darcy_svd%d'%k,
    ...
    np.save("time darcy svd %s k=%d per epoch"%(args.attention_type, k), a)
#    np.save("time darcy original galerkin per epoch", a)
#     time = result["time"]
#     time = np.mean(time)
# #     np.save("darcy mean time lin %s k=%d "%(args.attention_type, k), time)
#     np.save("darcy mean time original %s"%args.attention_type, time)
    ...
    np.save("best metric darcy svd %s k=%d"%(args.attention_type, k), val_metric)
    
    
    loss_train = result['loss_train']
    loss_val = result['loss_val']
#     np.save("darcy_%d_lin_%s_train_loss"%(k, args.attention_type), loss_train)
#     np.save('darcy_%d_lin_%s_val_loss'%(k, args.attention_type), loss_val)

#     np.save("darcy_original_%s_train_loss"%args.attention_type, loss_train)
#     np.save('darcy_original_%s_val_loss'%args.attention_type, loss_val)
...
#    np.save("inference time darcy original %s"%args.attention_type, mean_syn)   
    
    np.save("inference time darcy svd %s k=%d"%(args.attention_type, k), mean_syn) 
...
#         preds = out_['preds']
#         attn = out_['attn_weights']
#     torch.save(attn, "darcy attention")    
